...
//   GET  /api/replay-annotations?demo_id=<uuid>           → toutes annotations user pour cette demo (toutes rounds)
//   GET  /api/replay-annotations?demo_id=<uuid>&public=1  → annotations publiques pour cette demo (toutes users)
//   POST /api/replay-annotations { demo_id, round_num, payload, is_public }
//        → upsert (1 row par user/demo/round)
//   DELETE /api/replay-annotations { demo_id, round_num }  → delete user's annotation for this round
//
// Auth : Bearer access token Supabase (frontend localStorage sb-*-auth-token)
// RLS gere les permissions cote DB : user voit/edit ses propres, sees publiques en read-only.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { demo_id, round_num, payload = {}, is_public = false } = body || {};
      ...
      const { data, error } = await sb.from('fv_annotations').upsert({
        user_id: user.id,
        demo_id,
        round_num,
        payload,
        ...
        .select('id, round_num, is_public, updated_at')
        .single();
